 ...
 */
export function determineTierChangeReason(fromTier, toTier, isReactivation = false, tierRanks = {}, freeBaseTierId = 'LIGHT') {
    if (isReactivation)
        return 'reactivation';
    if (!fromTier)
        return 'signup';
    ...
    if (toTier.toUpperCase() === freeBaseTierId.toUpperCase() && fromRank > 0)
        return 'churn';
    if (toRank > fromRank)
        return 'upgrade';
    if (toRank < fromRank)
